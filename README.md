...
3. 点击「开始下一波」主动开波（提前开波有**速度分**加成）。
4. 撑过第 30 波的首领即获胜；漏怪扣除基地血量，归零则失败。
5. 登录后可将成绩与回放上传至**排行榜**。

> 计分 = 基础分（经济与击杀） × 高手倍率（1.0–2.0，操作越强越高）+ 速度分（开波节奏）。
...
欢迎 Issue 与 PR。改动游戏逻辑后，建议至少在浏览器中完成一次「选图 → 放塔 → 开波 → 通关」的冒烟验证（控制台应无报错）。

## 📄 许可证

本项目以 [MIT 许可证](LICENSE) 开源。BGM 音频文件不包含在内，其版权归各自所有者。
